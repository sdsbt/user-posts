...
public class PostgresSQLRepository implements DatabaseRepository, SyncLogger {
    ...
    public int syncUsers(JsonNode users) throws SQLException {
        ...
        String sql = """
                   insert into post_data.users (id, name, username, email, phone, website, last_synced)
                   values (?,?,?,?,?,?,?)
    ...
    public int syncPosts(JsonNode posts) throws Exception {
        ...
        String sql = """
                insert into post_data.posts(id, user_id, title, body, last_synced)
                values(?,?,?,?,?)
                on conflict (id) DO UPDATE SET
    ...
    public int syncComments(JsonNode comments) throws Exception {
        ...
        String sql = """
                insert into post_data.comments(id, post_id, name, email, body, last_synced)
                values(?,?,?,?,?,?)
                on conflict (id) do update set
            ...
            try(Connection conn = getConnection();
                ...
                for (JsonNode comment : comments) {
                    ...
                    pstmt.setString(5, comment.get("body").asText());
    ...
    public void logSyncStart() throws SQLException {
        String sql = "insert into post_data.sync_log (sync_time, status) values (?, 'STARTED')";
        try(Connection conn = getConnection();
            PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    public void logSyncComplete(int recordCount) throws SQLException {
        String sql = """
                        insert into post_data.sync_log (sync_time, status, record_count)
                        values (?, 'SUCCESS', ?)
                     """;
        try(Connection conn = getConnection();
            PreparedStatement pstmt = conn.prepareStatement(sql)){
    ...
    public void logSyncError(String error) throws SQLException {
        String sql = "INSERT INTO post_data.sync_log (sync_time, status, error_message) VALUES (?, 'FAILED', ?)";
